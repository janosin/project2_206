...
def grab_headlines():
    ...
    tags = soup.find("div", class_="view view-most-read view-id-most_read view-display-id-panel_pane_1 view-dom-id-99658157999dd0ac5aa62c2b284dd266")
...
def get_umsi_data():
    ...
    for i in range(13):
        ...
        tags = soup.find_all("div", class_="ds-1col node node-person node-teaser view-mode-teaser clearfix")
    
    
    
        for item in tags:
            y = item.find("div", class_ = "field field-name-title field-type-ds field-label-hidden").text
    
            t = item.find_all("div", class_ = "field field-name-field-person-titles field-type-text field-label-hidden")
            title = ""
            for tle in t:
                title =  (tle.text)
            umsi_titles[y] = title
    
        next_url = soup.find("li", class_ ="pager-next last")
        end_url = ""
    
        if i < 12:
            for bitch in next_url:
                    end_url = bitch.get("href")
                    base_url = "https://www.si.umich.edu" + end_url 
